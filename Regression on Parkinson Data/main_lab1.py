...
#%% Plotting :
features = ['age','sex','motor_UPDRS','total_UPDRS','Jitter(%)','Jitter(Abs)','Jitter:RAP','Jitter:PPQ5','Jitter:DDP','Shimmer','Shimmer(dB)','Shimmer:APQ3','Shimmer:APQ5','Shimmer:APQ11','Shimmer:DDA','NHR','HNR','RPDE','DFA','PPE']
...
m.comparison('y VS y\u0302 training - LLS','y\u0302 training','y training', y_train, x_train, mean[F],std[F])  #sto estraendo la media e la varianza della colonna total updrs
m.comparison('y VS y\u0302 test - LLS','y\u0302 test','y test', y_test, x_test, mean[F],std[F])  #sto estraendo la media e la varianza della colonna total updrs
...
c.comparison('y VS y\u0302 training - Conjugate','y\u0302 training','y training', y_trr, x_train, mean[F],std[F])
c.comparison('y VS y\u0302 test - Conjugate','y\u0302 test','y test', y_test, x_test, mean[F],std[F])
c.hist_plus_val(y_trr, x_train, y_txt, x_test, y_valid, x_val, mean[F],std[F], ' - CGA')
...
s.comparison('y VS y\u0302 test - SGD with ADAM','y\u0302 test','y test', y_test, x_test, mean[F],std[F])
s.hist_plus_val(y_trr, x_train, y_txt, x_test, y_valid, x_val, mean[F],std[F], ' - SGwA')
sgwa_stat=s.stat_u()
R2[2]=s.determ_coeff()
...
r.comparison('y VS y\u0302 test - Ridge','y\u0302 test','y test', y_test, x_test, mean[F],std[F])
r.hist_plus_val(y_trr, x_train, y_txt, x_test, y_valid, x_val, mean[F],std[F], ' - Ridge')
R2[3]=r.determ_coeff()
ridge_stat=r.stat_u()
